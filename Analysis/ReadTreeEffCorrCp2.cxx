...
void ReadTreeEffCorrCp2(const char* fname = "pp/tree/AnalysisResults_LHC17pq_afterCalib", const char* ofname = "o17_30sub_", const int iVarMin = 0, const int iVarMax = 1, const int tree_number = 1)
{
  ...
  for (int iC = 0; iC < 2; ++iC){
    ...
    for (int iCent = 0; iCent < kNCentBins; ++iCent){
      for (int iEta = 0; iEta < kNEtaBins; ++iEta){
        for (int iS = 0; iS < N_SAMPLE; ++iS){
          for (int iVar{67}; iVar < 68; ++iVar){
            if (fEffK){
              hEffK[iC][iCent][iEta][iS][0] = (TH1D*)fEffK->Get(Form("subsample_%d_var_%d/h%sEff%s_%d_%d_%d", 1, iVar, kAntiMatterLabel[iC], kPartLabel[0], iCent, 0, iVar));
            }
          }
          // TO BE IMPROVED -> CENTRALITY DIFFERENTIAL ESTIMATE OF XI EFFICIENCY (ALSO BDT)
          for (int iVar{7}; iVar < 8; ++iVar){
            if (fEffXi){
              hEffXi[iC][iCent][iEta][iS][0] = kUseBdtInMC || kUseKaonXiEff ? (TH1D*)fEffK->Get(Form("subsample_%d_var_%d/h%sEff%s_%d_%d_%d", 1, iVar, kAntiMatterLabel[iC], kPartLabel[1], iCent, 0, iVar)) : (TH1D*)fEffXi->Get(Form("fPreselEff_vs_pt_%s_%.0f_%.0f", kAntiMatterLabelML[iC], 0., 90.)); //kCentBins[iCent], kCentBins[iCent + 1]));
            }
          }
        }
      }
    ...
    for (int iVar{iVarMin}; iVar < iVarMax; ++iVar)
    {
      ...
      for (auto k_tmp : *k){
          ...
          )
        {
          ...
          double eff = fEffK ? hEffK[im][ic - 1][ie - 1][iS][0]->GetBinContent(hEffK[im][ic - 1][ie - 1][iS][iVar - iVarMin]->FindBin(std::abs(k_tmp.fPt))) : kDummyEffK;
      ...
      if (iVar < kNMassCuts * kNBdtCuts){
        for (auto xi_tmp : *xi){
            ...
            )
          {
            ...
            double eff = fEffXi ? hEffXi[im][ic - 1][ie - 1][iS][0]->GetBinContent(hEffXi[im][ic - 1][ie - 1][iS][iVar - iVarMin]->FindBin(std::abs(xi_tmp.fPt))) : kDummyEffXi;
