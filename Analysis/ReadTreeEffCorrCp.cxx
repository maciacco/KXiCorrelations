...
void ReadTreeEffCorrCp(const char* fname = "tree_data_full/part_merging_True/%s_AnalysisResults", const char* ofname = "o15_", const int tree_number = 1)
{
  ...
  TNtuple evtTuple("evtTuple", "evtTuple", "cent:q1kP:q1kN:q2kP:q2kN:q1xiP:q1xiN:q2xiP:q2xiN:trk:V0M");
  ...
  for (int iC = 0; iC < 2; ++iC){
    ...
    for (int iE = 0; iE < kNEtaBins; ++iE){
      hNsigmaITS[iC][iE] = new TH3F(Form("h%sNsigmaITS_%d", kAntiMatterLabel[iC], iE), ";Centrality (%);#it{p}_{T} (GeV/#it{c});#it{n}#sigma_{ITS} (a.u.)", 10, 0, 100, 30, 0, 3, 120, -6, 6);
      hNsigmaTPC[iC][iE] = new TH3F(Form("h%sNsigmaTPC_%d", kAntiMatterLabel[iC], iE), ";Centrality (%);#it{p}_{T} (GeV/#it{c});#it{n}#sigma_{TPC} (a.u.)", 10, 0, 100, 30, 0, 3, 120, -6, 6);
      hNsigmaTOF[iC][iE] = new TH3F(Form("h%sNsigmaTOF_%d", kAntiMatterLabel[iC], iE), ";Centrality (%);#it{p}_{T} (GeV/#it{c});#it{n}#sigma_{TOF} (a.u.)", 10, 0, 100, 30, 0, 3, 120, -6, 6);
    }

    hBDTOut[iC] = new TH3F(Form("h%sBDTOutXi", kAntiMatterLabel[iC]), ";Centrality (%);#it{p}_{T} (GeV/#it{c});BDT out", 10, 0, 100, 8, 0, 4, 1000, 0, 1);
  ...
  for (Long64_t i = 0; i < nEntries; ++i){
    ...
    for (auto k_tmp : *k){
      ...
      #endif // CLOSURE_TEST
      if (
          ( ( ( ((k_tmp.fCutBitMap & kCutDCA[1]) == kCutDCA[1]) || ((k_tmp.fCutBitMap & kCutDCA2[1]) == kCutDCA2[1]) ) && kRequireDCAcut[1] ) || !kRequireDCAcut[1] ) &&
          ( ( ((k_tmp.fCutBitMap & kCutTPCcls[1]) == kCutTPCcls[1] || (k_tmp.fCutBitMap & kCutTPCcls2[1]) == kCutTPCcls2[1]) && kRequireTPCclsCut[1] ) || !kRequireTPCclsCut[1]) &&
          ( ( ((k_tmp.fCutBitMap & kCutChi2[1]) == kCutChi2[1] || (k_tmp.fCutBitMap & kCutChi22[1]) == kCutChi22[1]) && kRequireChi2Cut[1] ) || !kRequireChi2Cut[1] ) &&
          std::abs(k_tmp.fPt) > kPtLowLimitK && std::abs(k_tmp.fPt) < kTOFptCut &&
          (std::abs(k_tmp.fEta) < kEtaCut)
          &&
          ((std::abs(k_tmp.fPt) < kTPCptCut && (k_tmp.fNsigmaTPC > kNsigmaTPCcutAsym[1][0] && k_tmp.fNsigmaTPC < kNsigmaTPCcutAsym[1][1])) ||
          (std::abs(k_tmp.fPt) > kTPCptCut && (std::abs(k_tmp.fPt) < kTOFptCut) && (std::abs(k_tmp.fNsigmaTPC) < kNsigmaTPCcutPresel) && (k_tmp.fNsigmaTOF > kNsigmaTOFcutAsym[1][0] && k_tmp.fNsigmaTOF < kNsigmaTOFcutAsym[1][1])))
        )
      {
    ...
    for (auto xi_tmp : *xi){
        ...
        )
      {
        ...
        hBDTOut[im]->Fill(cent, std::abs(xi_tmp.fPt), xi_tmp.fBdtOut);
      }
    }
    for (int iM = 0; iM < 2; ++iM){
      #ifdef CLOSURE_TEST
    ...
    for (int iM = 0; iM < 2; ++iM){
      ...
      for (int iC = 0; iC < 2; ++iC){ // loop over efficiency correction
        ...
        //if (!(i%1000000))std::cout <<  qXi_1[ic_sm-1][iC][iM][iS] << std::endl;
    ...
    evtTuple.Fill(cent, qK_1_tmp[1][1], qK_1_tmp[0][1], qK_2_tmp[1][1], qK_2_tmp[0][1], qXi_1_tmp[0][1], qXi_1_tmp[1][1], qXi_2_tmp[0][1], qXi_2_tmp[1][1], 0, 0);
  }
