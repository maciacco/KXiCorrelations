...
void Analysis_finalBinning_c2c1K()
{
  TFile f(Form("out_sys_%s_finalBinning_%s.root", period, kC2c1 ? "c2c1_k" : ""), "recreate");
  ...
  for (int i{0}; i < kNCentBinsAnalysis; ++i){
    ...
    else if (kC2c1) hSys[i] = new TH1D(Form("hSys_%d", i), ";C_{2}/C_{1};Entries", 2000, .0, 2.);
  }
  for(int iVar = 0; iVar < 1800; ++iVar)
  {
    ...
    for(int i = 1; i < kNCentBinsAnalysis; i++)
    {
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        if (kC2c1) rhomean = rhomean + ( ( c2k_pn[sample][i-1] ) / ( c1k_n[sample][i-1] + c1k_p[sample][i-1] ));
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        if (kC2c1) rhorms = rhorms + TMath::Power(rhomean - ( c2k_pn[sample][i-1] ) / ( c1k_n[sample][i-1] + c1k_p[sample][i-1] ), 2.0);
