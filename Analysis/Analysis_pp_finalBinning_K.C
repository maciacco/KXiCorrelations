...
void Analysis_pp_finalBinning_K()
{
  gStyle->SetOptStat(0);
  TFile f(kC2c1 ? "out_sys_17_finalBinning_k_p.root" : "out_sys_17_finalBinning.root", "recreate");
   ...
   // if (((iVar%45)/45.) > 0.01) continue;
    ...
    for(int i = 1; i < kNCentBinsAnalysis; i++)
    {
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        else if (kC2c1) rhomean = rhomean + ( ( c1k_p[sample][i-1] ) );
        ...
        if (iVar == (kC2c1 ? 52 : 1012)) hStat[i - 1]->Fill( ( c1k_p[sample][i-1] ) );
      }
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        else if (kC2c1) rhorms = rhorms + TMath::Power(rhomean - ( ( c1k_p[sample][i-1] ) ), 2.0);
      }
  ...
  c.Write();
  cSys.Write();
  cSys.Print(kC2c1 ? "cSys_pp_k_p.pdf" : "cSys_pp.pdf");
  f.Close();
}
