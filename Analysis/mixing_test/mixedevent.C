void mixedevent()
{
  ...
  for(int sample=0;sample<16;sample++)
    {
      ...
      float **q3=new float*[100];
      float **qP1=new float*[100];
      float **qP2=new float*[100];
      float **qP3=new float*[100];
      for(int i = 0; i < 100; i++)
	{
      ...
      for (int j=0;j<total_event;j++)
	{
	  ...
	  q1[centrality][evt[centrality]]=arg[5];
	  q2[centrality][evt[centrality]]=arg[7];
    
	  qP1[centrality][evt[centrality]]=arg[1];
	  qP2[centrality][evt[centrality]]=arg[3];
	  evt[centrality]=evt[centrality]+1;
  ...
  int range;  
  for(int CNTCLS=0;CNTCLS<90;CNTCLS++)
    {
      ...
      for(int evtcentclass=0;evtcentclass<range;evtcentclass++)
	{
	  ...
	  q1_xi->Fill(CNTCLS,q1[CNTCLS][evtcentclass]);
	  q2_xi->Fill(CNTCLS,q2[CNTCLS][evtcentclass]);
	  q1square_xi->Fill(CNTCLS,q1[CNTCLS][evtcentclass]*q1[CNTCLS][evtcentclass]);

	  q1_kaon->Fill(CNTCLS,qP1[CNTCLS][random]);
	  q2_kaon->Fill(CNTCLS,qP2[CNTCLS][random]);
	  q1square_kaon->Fill(CNTCLS,qP1[CNTCLS][random]*qP1[CNTCLS][random]);
	  
	  q1_xi_kaon->Fill(CNTCLS,qP1[CNTCLS][random]*q1[CNTCLS][evtcentclass]);
	}
    }
  TFile *fout=new TFile(Form("output_mixedevent_smallBin_sample%d.root",sample),"RECREATE");
  fout->cd();
  q1_xi->Write();
  q2_xi->Write();
  q1square_xi->Write();
