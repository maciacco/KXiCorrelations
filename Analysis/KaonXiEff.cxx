...
void KaonXiEff(const char* inFileName = "oo_limit_21d6_strinj", const char* outFileName = "KaonXiEff_21d6_strinj"){
  gStyle->SetOptStat(0);
  ...
  for (int iS = 0; iS < N_SAMPLE; ++iS){
    ...
    for (int iP = 0; iP < 2; ++iP){
      for (int iM = 0; iM < 2; ++iM){
        ...
        TCanvas ce(Form("c%sEff_%s", kAntiMatterLabel[iM], kPartLabelExtend[iP]), Form("c%sEff_%s", kAntiMatterLabel[iM], kPartLabelExtend[iP]), 500, 500);
        ...
        for (int iC = 0; iC < kNCentBins; ++iC){
          for (int iE = 0; iE < kNEtaBins; ++iE){
            ...
            hEff[iC][iE]->SetName(Form("h%sEff%s_%d_%d", kAntiMatterLabel[iM], kPartLabel[iP], iC, iE));
            ...
            hEff[iC][iE]->GetXaxis()->SetRangeUser(0., iP == 1 ? 4. : 1.);
        ...
        leg.Draw("same");
        ce.Write();
        ce.Print(Form("c%sEff_%s.pdf", kAntiMatterLabel[iM], kPartLabel[iP]));
      }
    }
  }
  fMC->Close();
