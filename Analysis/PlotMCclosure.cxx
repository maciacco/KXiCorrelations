...
void PlotMCclosure(const char* inFileName = "outMC_LHC21d6_", const char* outFileName = "mcClosure_LHC21d_"){
    ...
    for (int iC = 0; iC < 2; ++iC){
      for (int iP = 0; iP < 2; ++iP){
        ...
        for (int iGR = 0; iGR < 2; ++iGR){
          h[iGR] = (TH1D*)in.Get(Form("h%s%s%sCentEffCorrCBWC_%s%s", kAntiMatterLabel[iM], kAvgVarLabel[iC], kPartLabel[iP], kCorrLabelUnderscore[iGR], kCorrLabel[iGR == 0 ? 2 : 0]));
          if (iGR == 0){
            h[iGR]->GetYaxis()->SetTitle(Form("C_{%d}(%s)", iC + 1, kParticleAntiparticleTextAM[iP][iM]));
          }
          h[iGR]->SetLineColor(colors[iGR]);
          h[iGR]->SetMarkerColor(colors[iGR]);
          c.cd();
  ...
  for (int iP = 0; iP < 2; ++iP){
    ...
    for (int iGR = 0; iGR < 2; ++iGR){
      h[iGR] = (TH1D*)in.Get(Form("hNet%s%sCentEffCorrCBWC_%s%s", "Var", kPartLabel[iP], kCorrLabelUnderscore[iGR], kCorrLabel[iGR + 1]));
      if (iGR == 0){
