...
void Analysis_pPb_finalBinning_Xi()
{
  gStyle->SetOptStat(0);
  TFile f(kC2c1 ? "out_sys_16_finalBinning_xi_p.root" : "out_sys_16_finalBinning.root", "recreate");
  ...
  for (int i{0}; i < kNCentBinsAnalysis; ++i){
    ...
    else if (kC2c1) hSys[i] = new TH1D(Form("hSys_%d", i), ";C_{2}/C_{1};Entries", 10, -1, -2/* i > 4 ? 1000 : 2000, 0.7, 1.3 */);
  }
  for (int i{0}; i < kNCentBinsAnalysis; ++i){
   ...
   // if ((iVar/135)%3 == 2) continue;
    ...
    for(int i = 1; i < kNCentBinsAnalysis; i++)
    {
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        else if (kC2c1) rhomean = rhomean + ( ( c1xi_p[sample][i-1] ));
        //rhomean = rhomean + ( c1xi_n[sample][i-1] );
        if (iVar == (kC2c1 ? 960 : 1012)) hStat[i - 1]->Fill(( c1xi_p[sample][i-1] ));
        //if (iVar == (kC2c1 ? 960 : 1012)) hStat[i - 1]->Fill((c11_pp[sample][i-1] + c11_nn[sample][i-1] - c11_pn[sample][i-1] - c11_np[sample][i-1]) / sqrt(c2xi_pn[sample][i-1] * c2k_pn[sample][i-1])/* (c1xi_n[sample][i-1] + c1xi_p[sample][i-1]) / (c1k_p[sample][i-1] + c1k_n[sample][i-1]) */);
      }
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        else if (kC2c1) rhorms = rhorms + TMath::Power(rhomean - ( c1xi_p[sample][i-1] ), 2.0);
      }
  ...
  for (int i{0}; i < kNCentBinsAnalysis - 1; ++i){
    ...
    hSysFrame[i] = new TH1D(Form("hSysFrame_%d", i), kC2c1 ? ";#it{C}_{2}/#it{C}_{1};Entries" : ";#it{#rho};Entries", 1000000, -0.1, 5.);
  ...
  c.Write();
  cSys.Write();
  cSys.Print(kC2c1 ? "cSys_pPb_xi_p.pdf" : "cSys_pPb.pdf");
  f.Close();
}
