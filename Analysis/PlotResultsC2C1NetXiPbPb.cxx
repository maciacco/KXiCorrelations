...
void PlotResultsC2C1NetXiPbPb(){
  ...
  TH2D frame("frame", ";#LTd#it{N}_{ch}/d#it{#eta}#GT;#it{#kappa}_{2}(#bar{#Xi}^{+} - #Xi^{#minus})/#it{#kappa}_{1}(#bar{#Xi}^{+} + #Xi^{#minus})", 1, 0, 80, 1, 0.90, 1.06);
  ...
  for (int iP = kNPointspp - 1; iP < kNPointsPbPb + kNPointspp - 1; ++iP){
    ...
    gSHM.AddPoint(iP < kNPointspp - 1 ? mult_shm_pp[iP] : centClExtSHM[iP - kNPointspp + 1], iP < kNPointspp - 1 ? shm_pp_c2c1[iP][0] : gSHM_BS_->GetPointY(iP - kNPointspp + 1));
    gSHM.SetPointError(iP - kNPointspp + 1, 0, iP < kNPointspp - 1 ? shm_pp_c2c1[iP][1] : gSHM_BS_->GetErrorY(iP - kNPointspp + 1));
  }
  ...
  for (int iP = 0; iP < kNPointsPbPb; ++iP){
    gHIJING.AddPoint(centClExt[iP], hijing_c2c1[iP][0]);
 ...
 // leg.AddEntry(gSHM_4, "TheFIST CSM, #it{V}_{C} = 4d#it{V}/d#it{y}", "f");
  ...
  leg.AddEntry(gEPOS_PbPb, "EPOS LHC, Pb#minusPb", "lf");
 ...
 //gPYTHIA_ANGANTYR_PPB.Draw("samee3l");
  ...
  t.SetTextFont(44);
  t.SetTextSize(30);
  t.DrawLatex(/* 35 */10, 0.93, "#sqrt{#it{s}_{NN}} = 5.02 TeV, |#it{#eta}| < 0.8");

  //t.DrawLatex(2.5, 0.940, "0.2 < #it{p}_{T} (K) < 1.0 GeV/#it{c}");
  t.DrawLatex(10, 0.92 /* 0.935 */, "1.0 < #it{p}_{T}(#Xi) < 3.0 GeV/#it{c}");

  std::cout << utils::chi2interp(gpp_stat, gpp_sys, gPYTHIA_CRQCD) << "\n";

  TFile o("c2c1_pbpb.root", "recreate");
  o.cd();
  //gData.Write();
  cResult.Write();
  cResult.Print("cC2C1_pbpb.pdf"/*.pdf"*/);
  gPbPb_stat->GetXaxis()->SetTitle("#LTd#it{N}_{ch}/d#it{#eta}#GT");
  gPbPb_stat->GetYaxis()->SetTitle("#it{#kappa}_{2}(#bar{#Xi}^{+} - #Xi^{#minus})/#it{#kappa}_{1}(#bar{#Xi}^{+} + #Xi^{#minus})");
  gPbPb_sys->GetXaxis()->SetTitle("#LTd#it{N}_{ch}/d#it{#eta}#GT");
  gPbPb_sys->GetYaxis()->SetTitle("#it{#kappa}_{2}(#bar{#Xi}^{+} - #Xi^{#minus})/#it{#kappa}_{1}(#bar{#Xi}^{+} + #Xi^{#minus})");
  gPbPb_sys->Write();
  gPbPb_stat->Write();
  gEPOS_PbPb->SetName("gEPOS_LHC_PbPb");
  gEPOS_PbPb->GetXaxis()->SetTitle("#LTd#it{N}_{ch}/d#it{#eta}#GT");
  gEPOS_PbPb->GetYaxis()->SetTitle("#it{#kappa}_{2}(#bar{#Xi}^{+} - #Xi^{#minus})/#it{#kappa}_{1}(#bar{#Xi}^{+} + #Xi^{#minus})");
  gEPOS_PbPb->Write();
  gHIJING.SetName("gHIJING");
  gHIJING.GetXaxis()->SetTitle("#LTd#it{N}_{ch}/d#it{#eta}#GT");
  gHIJING.GetYaxis()->SetTitle("#it{#kappa}_{2}(#bar{#Xi}^{+} - #Xi^{#minus})/#it{#kappa}_{1}(#bar{#Xi}^{+} + #Xi^{#minus})");
  gHIJING.Write();
  // gPYTHIA_ANGANTYR.SetName("gPYTIHA_Angantyr");
  // gPYTHIA_ANGANTYR.GetXaxis()->SetTitle("#LTd#it{N}_{ch}/d#it{#eta}#GT");
  // gPYTHIA_ANGANTYR.GetYaxis()->SetTitle("#it{#kappa}_{2}(#bar{#Xi}^{+} - #Xi^{#minus})/#it{#kappa}_{1}(#bar{#Xi}^{+} + #Xi^{#minus})");
  // gPYTHIA_ANGANTYR.Write();
  gSHM.SetName("gGammasSHM");
  gSHM.GetXaxis()->SetTitle("#LTd#it{N}_{ch}/d#it{#eta}#GT");
  gSHM.GetYaxis()->SetTitle("#it{#kappa}_{2}(#bar{#Xi}^{+} - #Xi^{#minus})/#it{#kappa}_{1}(#bar{#Xi}^{+} + #Xi^{#minus})");
  gSHM.Write();
  o.Close();
  f.Close();
  //f2.Close();
