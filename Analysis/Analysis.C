...
void Analysis()
{
  TFile f("out_sys_18_new_3.root", "recreate");
  ...
  for (int i{0}; i < kNCentBinsAnalysis; ++i){
    //hSys[i] = new TH1D(Form("hSys_%d", i), ";#rho;Entries", i == 0 ? 200 : 100, -0.05, -0.01);
    //hSys[i] = new TH1D(Form("hSys_%d", i), ";#rho;Entries", 10000, -0, 1000);
    hSys[i] = new TH1D(Form("hSys_%d", i), ";C_{2}/C_{1};Entries", 100000, -1.0, 1.0);
  }
  for(int iVar = 0; iVar < 1350; ++iVar)
  {
    ...
    if ((iVar%90)/90. > 0.001 && kC2c1) continue;
    ...
    {
      TFile *fin = new TFile(Form("%s/output_sys_dir/18qr_2/output_sys_18qr_2_%d_%d.root", kResDir, sample, iVar));
      ...
      double xbins[] = {0., 10., 30., 40., 50., 70., 80.};
    ...
    for(int i = 1; i < kNCentBinsAnalysis; i++)
    {
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        if (kC2c1) rhomean = rhomean + ( ( c2xi_pn[sample][i-1] ) / ( c1xi_n[sample][i-1] + c1xi_p[sample][i-1] ));
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        if (kC2c1) rhorms = rhorms + TMath::Power(rhomean - ( c2xi_pn[sample][i-1] ) / ( c1xi_n[sample][i-1] + c1xi_p[sample][i-1] ), 2.0);
