...
void PlotResults_K(){
  ...
  TFile fpp("out_sys_17_finalBinning_k.root"); // 17pq
  TFile fpPb("out_sys_16_finalBinning_k.root"); // 17pq
  TFile f("out_sys_PbPb_finalBinning_k.root"); // 15o+18qr
  ...
  TH2D frame("frame", ";#LTd#it{N}_{ch}/d#it{#eta}#GT;#it{#kappa}_{1}(K^{+})", 1, 1.5, 2500, 1, 0.1, 150.);
  ...
  gEPOS_PbPb = (TGraphErrors*)fEPOSPbPb.Get("Gkp");
  ...
  t.SetTextFont(44);
  t.SetTextSize(23);
  t.DrawLatex(/* 35 */2.3, 15, "#sqrt{#it{s}_{NN}} = 5.02 TeV, |#it{#eta}| < 0.8");

  t.DrawLatex(2.3, 10, "0.2 #leq #it{p}_{T} (K) < 1.0 GeV/#it{c}");
  //t.DrawLatex(2.5, 0.96, "1.0 #leq #it{p}_{T} (#Xi) < 3.0 GeV/#it{c}");

  TFile o("final_plot_k.root", "recreate");
  o.cd();
  gData.Write();
  cResult.Write();
  cResult.Print("c_k.pdf");
  o.Close();
  f.Close();
  //f2.Close();
}