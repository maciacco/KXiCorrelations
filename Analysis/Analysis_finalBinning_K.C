...
void Analysis_finalBinning_K()
{
  TFile f(Form("out_sys_%s_finalBinning_%s.root", period, kC2c1 ? "_k_p" : ""), "recreate");
  ...
  for(int iVar = 0; iVar < 1800; ++iVar)
  {
    ...
    for(int i = 1; i < kNCentBinsAnalysis; i++)
    {
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        if (kC2c1) rhomean = rhomean + ( ( c1k_p[sample][i-1] ));
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        if (kC2c1) rhorms = rhorms + TMath::Power(rhomean - ( c1k_p[sample][i-1] ), 2.0);
