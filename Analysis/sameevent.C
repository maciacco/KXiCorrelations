...
void sameevent(int smpl = 0, int iVarMin = 0, int iVarMax = 1)
{
  ...
  for (int iVar = iVarMin; iVar < iVarMax; ++iVar){ 
    ...
    TFile *fin = TFile::Open(Form("/data/mciacco/KXiCorrelations/output_sys_dir/17pq_3/o_%d_var_%d.root"/* , kResDir */, sample + 1, iVar / ( kNTpcClsCuts * kNDcaCuts * kNPidCuts * kNChi2Cuts ))); 
    TFile *fin2 = TFile::Open(Form("/data/mciacco/KXiCorrelations/output_sys_dir/17pq_3/o_%d_var_%d.root"/* , kResDir */, sample + 1, iVar % ( kNTpcClsCuts * kNDcaCuts * kNPidCuts * kNChi2Cuts ))); 
    ...
    TFile fout(Form("%s/output_sys_dir/17pq_3/output_sys_17pq_3_%d_%d.root", kResDir, sample, iVar), "recreate");
