...
void Analysis_pPb()
{
  ...
  TFile f(kC2c1 ? "out_sys_16_c2c1_3.root" : "out_sys_16_4.root", "recreate");
  ...
  for (int i{0}; i < kNCentBinsAnalysis; ++i){
    if (kRho) hSys[i] = new TH1D(Form("hSys_%d", i), ";#rho;Entries", /* 160, -0.04, 0.0 */100000, -1.0, 1.0);
    //hSys[i] = new TH1D(Form("hSys_%d", i), ;#rho;Entries", 120, -0.05, 0.);
    else if (kC2c1) hSys[i] = new TH1D(Form("hSys_%d", i), ";C_{2}/C_{1};Entries", 600, 0.7, 1.3);
  }
  for (int i{0}; i < kNCentBinsAnalysis; ++i){
  ...
  for(int iVar = 0; iVar < 810; ++iVar)
  {
    if ( (((iVar%90)/90.) > 0.005)  && kC2c1 ) continue;
    else if (!kC2c1 && kRho){
   ...
   // if ((iVar/135)%3 == 2) continue;
    ...
    {
      TFile *fin = new TFile(Form("%s/output_sys_dir/16qt/output_sys_16qt_%d_%d.root", kResDir, sample, iVar));
      ...
      double xbins[] = {0., 10., 20., 40., 60., 80., 100.};
      q1xikaon_pp = (TProfile*)q1xikaon_pp->Rebin(6, Form("var_%d/q1_xi_kaon_pp", iVar), xbins);
      q1xikaon_pn = (TProfile*)q1xikaon_pn->Rebin(6, Form("var_%d/q1_xi_kaon_pn", iVar), xbins);
      q1xikaon_np = (TProfile*)q1xikaon_np->Rebin(6, Form("var_%d/q1_xi_kaon_np", iVar), xbins);
      q1xikaon_nn = (TProfile*)q1xikaon_nn->Rebin(6, Form("var_%d/q1_xi_kaon_nn", iVar), xbins);
      q1xixi_pn = (TProfile*)q1xixi_pn->Rebin(6, Form("var_%d/q1_xi_xi_pn", iVar), xbins);
      q1kk_pn = (TProfile*)q1kk_pn->Rebin(6, Form("var_%d/q1_kaon_kaon_pn", iVar), xbins);
      q1xi_p = (TProfile*)q1xi_p->Rebin(6, Form("var_%d/q1_xi_p", iVar), xbins);
      q2xi_p = (TProfile*)q2xi_p->Rebin(6, Form("var_%d/q2_xi_p", iVar), xbins);
      q1xi_n = (TProfile*)q1xi_n->Rebin(6, Form("var_%d/q1_xi_n", iVar), xbins);
      q2xi_n = (TProfile*)q2xi_n->Rebin(6, Form("var_%d/q2_xi_n", iVar), xbins);
      q1squarexi_p = (TProfile*)q1squarexi_p->Rebin(6, Form("var_%d/q1square_xi_p", iVar), xbins);
      q1squarexi_n = (TProfile*)q1squarexi_n->Rebin(6, Form("var_%d/q1square_xi_n", iVar), xbins);
      q1kaon_p = (TProfile*)q1kaon_p->Rebin(6, Form("var_%d/q1_kaon_p", iVar), xbins);
      q2kaon_p = (TProfile*)q2kaon_p->Rebin(6, Form("var_%d/q2_kaon_p", iVar), xbins);
      q1kaon_n = (TProfile*)q1kaon_n->Rebin(6, Form("var_%d/q1_kaon_n", iVar), xbins);
      q2kaon_n = (TProfile*)q2kaon_n->Rebin(6, Form("var_%d/q2_kaon_n", iVar), xbins);
      q1squarekaon_p = (TProfile*)q1squarekaon_p->Rebin(6, Form("var_%d/q1square_kaon_p", iVar), xbins);
      q1squarekaon_n = (TProfile*)q1squarekaon_n->Rebin(6, Form("var_%d/q1square_kaon_n", iVar), xbins);
    ...
    for(int i = 1; i < kNCentBinsAnalysis; i++)
    {
      ...
      for(int sample = 0; sample < kNSample; sample++)
      {
        ...
        if (iVar == (kC2c1 ? 360 : 442)) hStat[i - 1]->Fill(( c2xi_pn[sample][i-1] ) / ( c1xi_n[sample][i-1] + c1xi_p[sample][i-1] ));
        //if (iVar == (kC2c1 ? 360 : 442)) hStat[i - 1]->Fill(/* (c11_pp[sample][i-1] + c11_nn[sample][i-1] - c11_pn[sample][i-1] - c11_np[sample][i-1]) / sqrt(c2xi_pn[sample][i-1] * c2k_pn[sample][i-1]) */(c1xi_n[sample][i-1] + c1xi_p[sample][i-1]) / (c1k_p[sample][i-1] + c1k_n[sample][i-1]));
      }
  ...
  for (int i{0}; i < 6; ++i){
    ...
    hSys[i]->SetTitle(Form("mult. class %d-%d%%", kMultClasses[i][0], kMultClasses[i][1]));
    cSys.cd(i + 1);
    hSys[i]->Draw("histo");
    cSys.Update();
    hStat[i]->Write();
