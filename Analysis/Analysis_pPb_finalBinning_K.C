...
void Analysis_pPb_finalBinning_K()
{
  gStyle->SetOptStat(0);
  TFile f(kC2c1 ? "out_sys_16_finalBinning_k_p.root" : "out_sys_16_finalBinning.root", "recreate");
  ...
  c.Write();
  cSys.Write();
  cSys.Print(kC2c1 ? "cSys_pPb_k.pdf" : "cSys_pPb.pdf");
  f.Close();
}
